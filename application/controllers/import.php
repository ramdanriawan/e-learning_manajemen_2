...
class Import extends CI_Controller {
    ...
    public function materi()
    {
        ...
        $target_file      = './upload/temp/';
        $buat_folder_temp = !is_dir($target_file) ? @mkdir("./upload/temp/") : false;
        $file_folder = $target_file.$_FILES['import_excel']['name'];
        $file_folder = substr($file_folder, 2);
        // print_r($_FILES); die();
        move_uploaded_file($_FILES["import_excel"]["tmp_name"], $file_folder);
        
        $file   = explode('.', $_FILES['import_excel']['name']);
        $length = count($file);
        

        // jika yang diupload adalah bukan file excel maka langsung redirect ke halaman materi
        if (in_array($file[$length - 1], ['pdf', 'doc', 'docx'])) {
            // print_r(); die();

            $data = "'$p[id_mapel]', $p[id_guru], '$file_folder'";
            
            $strq = "INSERT INTO m_materi (id_mapel, id_guru, materi) VALUES ($data)";

            $this->db->query($strq);

            goto redirect;
        }
        elseif (in_array($file[$length - 1], ['xlsx', 'xls'])) {
            ...
            $data = array();
            for ($j = $idx_baris_mulai; $j <= $idx_baris_selesai; $j++) {
                $materi = $_sheet->getCell("A" . $j)->getValue();

                if ($materi != "") {
                    $data[] = "('$p[id_mapel]','$p[id_guru]','$materi')";
                }
            }
            if (!count($data)) {
                die('tidak ada materi yang diimport');
            }

            $strq = "INSERT INTO m_materi (id_mapel, id_guru, materi) VALUES " . implode(",", $data);
            // print_r($strq); die();

            $this->db->query($strq);
        } else {
            exit('Format file tidak di dukung...'); //pesan error tipe file tidak tepat
        }
