...
class Adm extends CI_Controller
{
    ...
    {
        $this->cek_aktif();
        cek_hakakses(['admin'], $this->session->userdata('admin_level'));
        ...
        if ($uri3 == 'det') {
            ...
            foreach ($q_datanya as $d) {
                ...
                $data_ok[] = '<div class="btn-group">
                          <a href="#" onclick="return m_siswa_e(' . $d['id'] . ');" class="btn btn-info btn-xs"><i class="fa fa-edit" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_siswa_h(' . $d['id'] . ');" class="btn btn-danger btn-xs"><i class="fa fa-trash" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Hapus</a>
                         ';
        ...
        if ($uri3 == 'det') {
            ...
            foreach ($q_datanya as $d) {
                ...
                $data_ok[3] = '<div class="btn-group">
                          <a href="#" onclick="return m_guru_e(' . $d['id'] . ');" class="btn btn-info btn-xs"><i class="fa fa-edit" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_guru_h(' . $d['id'] . ');" class="btn btn-danger btn-xs"><i class="fa fa-trash" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Hapus</a>
                          <a href="#" onclick="return m_guru_matkul(' . $d['id'] . ');" class="btn btn-success btn-xs"><i class="fa fa-th-list" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;MaPel Diampu</a>
                         ';
    ...
    {
        $this->cek_aktif();
        cek_hakakses(array('admin', 'siswa', 'guru'), $this->session->userdata('admin_level'));
        ...
        if ($uri3 == 'det') {
            ...
            if ($p->id != 0) {
                ...
                $ket = 'tambah';
                $this->db->query("INSERT INTO m_mapel(id, nama) VALUES('', '" . bersih($p, 'nama') . "')");
            }
            ...
            foreach ($q_datanya as $d) {
                ...
                $data_ok[2] = '<div class="btn-group">
                          <a href="#" onclick="return m_mapel_e(' . $d['id'] . ');" class="btn btn-info btn-xs"><i class="fa fa-edit" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_mapel_h(' . $d['id'] . ');" class="btn btn-danger btn-xs"><i class="fa fa-trash" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Hapus</a>
                         ';
    ...
    /* == GURU == */
    public function m_soal($jenis_soal = null)
    {
        ...
        if ($uri3 == 'det') {
            ...
            foreach ($_FILES as $k => $v) {
                ...
                if ($file_error != 0) {
                    ...
                    $nama_file[$k] = '';
                    $tipe_file[$k] = '';
                } elseif (!in_array($file_type, $allowed_type)) {
            ...
            redirect("adm/m_soal/$p[jenis_soal]");
        } elseif ($uri3 == 'edit') {
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $d) {
                    ...
                    } elseif ($file_ada == 'ada' && $tipe_media == 'video') {
                        $tampil_media = '<<< Ada media Videonya >>>';
                    } elseif ($file_ada == 'ada' && $tipe_media == 'gambar') {
            ...
            echo $html . '<script>print(); close();</script>';
            exit;
        } elseif ($uri3 == 'data') {
            $start  = $this->input->post('start');
            $length = $this->input->post('length');
            ...
            } elseif ($a['sess_level'] == 'admin') {
                $wh = '';
            }

            // cek user akan melihat data soal latihan atau data soal untuk ujian (uri4 adalah jenis soalnya)
            // die($jenis_soal);
            
            $wh .= "jenis_soal='$uri4' AND ";
            ...
            foreach ($q_datanya as $d) {
                ...
                // jika usernya admin atau guru maka berikan hak akses untuk edit dan hapus
                if (in_array($this->session->userdata('admin_level'), ['admin', 'guru'])) {
                    $data_ok[4] = '<div class="btn-group">
                      <a href="' . base_url() . 'adm/m_soal/edit/' . $d['id'] . '" class="btn btn-info btn-xs"><i class="fa fa-edit" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
                      <a href="' . base_url() . 'adm/m_soal/hapus/' . $d['id'] . '" class="btn btn-danger btn-xs" onclick="return confirm(\'Anda Yakin..?\');"><i class="fa fa-trash" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Hapus</a>
                     ';
                }
    ...
    public function m_ujian()
    {
        ...
        // edited by
        $a["gurus"]  = obj_to_array($this->db->get("m_guru")->result(), 'id,nama');
        $a["mapels"] = obj_to_array($this->db->get("m_mapel")->result(), 'id,nama');
        ...
        if ($uri3 == 'det') {
            ...
            $ambil_data = $this->db->query("SELECT id FROM m_soal WHERE  jenis_soal='ujian' and  id_mapel = '" . bersih($p, 'mapel') . "' AND id_guru = '" . $a['sess_konid'] . "'")->num_rows();

            // tapi kalo admin yang mau nambahin
            if (in_array($a['sess_level'], ['admin'])) {
                // print_r(bersih($p, 'mapel')); die();
                $ambil_data = $this->db->query("SELECT id FROM m_soal WHERE  jenis_soal='ujian' and  id_mapel = '" . bersih($p, 'mapel') . "'")->num_rows();
                // die('total' . $ambil_data);
            }
            ...
            if ($ambil_data < $jml_soal_diminta) {
                ...
                if ($p->id != 0) {
                    ...
                    // print_r($p->id_guru); die();
                    // jika yang login adalah admin maka kon id adalah berdasarkan guru yang dipilih oleh adminnyaaaaaaaaa
                    if (in_array($this->session->userdata('admin_level'), ['admin'])) {
                        $a['sess_konid'] = $p->id_guru;
                    }
                    // print_r($a['sess_konid']); die();

                    $this->db->query("INSERT INTO tr_guru_tes
            ...
            foreach ($q_datanya as $d) {
                ...
                $data_ok[6] = '
		            	<div class="btn-group">
                          <a href="#" onclick="return m_ujian_e(' . $d['id'] . ');" class="btn btn-info btn-xs"><i class="fa fa-edit" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_ujian_h(' . $d['id'] . ');" class="btn btn-danger btn-xs"><i class="fa fa-trash" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Hapus</a>
                        </div>
	                         ';
    ...
    public function m_latihan()
    {
        ...
        if ($uri3 == 'det') {
            ...
            $ambil_data = $this->db->query("SELECT id FROM m_soal WHERE jenis_soal='latihan' and id_mapel = '" . bersih($p, 'mapel') . "' AND id_guru = '" . $a['sess_konid'] . "'")->num_rows();

            // tapi kalo admin yang mau nambahin
            if (in_array($a['sess_level'], ['admin'])) {
                // print_r(bersih($p, 'mapel')); die();
                $ambil_data = $this->db->query("SELECT id FROM m_soal WHERE  jenis_soal='latihan' and id_mapel = '" . bersih($p, 'mapel') . "'")->num_rows();
                // die('total' . $ambil_data);
            }
            ...
            $d_total_row = $this->db->query("SELECT a.id
		        	FROM tr_guru_tes a
		        	INNER JOIN m_mapel b ON a.id_mapel = b.id
		        	WHERE a.jenis_soal='latihan' and a.id_guru = '" . $a['sess_konid'] . "'
                    AND (a.nama_ujian LIKE '%" . $search['value'] . "%'
					OR b.nama LIKE '%" . $search['value'] . "%')")->num_rows();
            ...
            // print_r($q_datanya); die();
            // kalo yang login adalah admin
            if (in_array($this->session->userdata('admin_level'), ['admin'])) {
                $d_total_row = $this->db->query("SELECT a.id
                        FROM tr_guru_tes a
                        INNER JOIN m_mapel b ON a.id_mapel = b.id
                        WHERE  a.jenis_soal='latihan' and (a.nama_ujian LIKE '%" . $search['value'] . "%'
                        OR b.nama LIKE '%" . $search['value'] . "%')")->num_rows();
            ...
            foreach ($q_datanya as $d) {
                ...
                $data_ok[6] = '
		            	<div class="btn-group">
                          <a href="#" onclick="return m_ujian_e(' . $d['id'] . ');" class="btn btn-info btn-xs"><i class="fa fa-edit" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
                          <a href="#" onclick="return m_latihan_h(' . $d['id'] . ');" class="btn btn-danger btn-xs"><i class="fa fa-trash" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Hapus</a>
                        </div>
	                         ';
    ...
    {
        $this->cek_aktif();
        cek_hakakses(array('admin', '', 'guru', 'ortu'), $this->session->userdata('admin_level'));
        ...
        $wh_1 = $a['sess_level'] == 'admin' || $a['sess_level'] == 'ortu' ? '' : " AND a.id_guru = '" . $a['sess_konid'] . "'";
        ...
        if ($uri3 == 'det') {
            ...
            foreach ($q_datanya as $d) {
                ...
                // kalo yang login adalah admin atau guru maka bisa melakukan pembatalan
                if (in_array($this->session->userdata('admin_level'), ['admin', 'guru'])) {
                    $data_ok[5] = '<a href="' . base_url() . 'adm/h_ujian/batalkan_ujian/' . $d['id'] . '/' . $this->uri->segment(4) . '" class="btn btn-danger btn-xs" onclick="return confirm(\'Anda yakin...?\');"><i class="fa fa-trash" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Batalkan Ujian</a>';
                }
            ...
            $q_datanya = $this->db->query("SELECT a.*, b.nama AS mapel, c.nama AS nama_guru FROM tr_guru_tes a
	        	INNER JOIN m_mapel b ON a.id_mapel = b.id
	        	INNER JOIN m_guru c ON a.id_guru = c.id
                -- INNER JOIN tr_ikut_ujian d ON a.id = d.id_tes
    ...
    public function h_latihan()
    {
        ...
        if ($uri3 == 'det') {
            $a['detil_tes'] = $this->db->query("SELECT m_mapel.nama AS namaMapel, m_guru.nama AS nama_guru,
												...
												INNER JOIN m_mapel ON tr_guru_tes.id_mapel = m_mapel.id
												INNER JOIN m_guru ON tr_guru_tes.id_guru = m_guru.id
												WHERE jenis_soal='latihan' and tr_guru_tes.id = '$uri4'")->row();
            $a['statistik'] = $this->db->query("SELECT MAX(nilai) AS max_, MIN(nilai) AS min_, AVG(nilai) AS avg_
											FROM tr_ikut_ujian
											WHERE tr_ikut_ujian.id_tes = '$uri4'")->row();

            //$a['hasil'] = $this->db->query("")->result();
            $a['p'] = 'm_guru_tes_hasil_detil_latihan';
            // print_r($a['detil_tes']); die();
        } elseif ($uri3 == 'data_det') {
            $start  = $this->input->post('start');
            $length = $this->input->post('length');
            $draw   = $this->input->post('draw');
            ...
            foreach ($q_datanya as $d) {
                ...
                // kalo yang login adalah admin atau guru maka bisa melakukan pembatalan
                if (in_array($this->session->userdata('admin_level'), ['admin', 'guru'])) {
                    $data_ok[5] = '<a href="' . base_url() . 'adm/h_latihan/batalkan_latihan/' . $d['id'] . '/' . $this->uri->segment(4) . '" class="btn btn-danger btn-xs" onclick="return confirm(\'Anda yakin...?\');"><i class="fa fa-trash" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Batalkan Latihan</a>';
                }
            ...
            j($json_data);
            exit;
        } elseif ($uri3 == 'batalkan_latihan') {
            $this->db->query("DELETE FROM tr_ikut_ujian WHERE id = '$uri4'");
            redirect('adm/h_latihan/det/' . $uri5);
        } elseif ($uri3 == 'data') {
            $start  = $this->input->post('start');
            $length = $this->input->post('length');
            $draw   = $this->input->post('draw');
            ...
            $d_total_row = $this->db->query("SELECT a.id FROM tr_guru_tes a
	        	INNER JOIN m_mapel b ON a.id_mapel = b.id
	        	INNER JOIN m_guru c ON a.id_guru = c.id
	            WHERE  jenis_soal='latihan' and (a.nama_ujian LIKE '%$search[value]%' OR b.nama LIKE '%$search[value]%' OR c.nama LIKE '%$search[value]%') " . $wh_1 . '')->num_rows();
            //echo $this->db->last_query();
            ...
            $q_datanya = $this->db->query("SELECT a.*, b.nama AS mapel, c.nama AS nama_guru FROM tr_guru_tes a
	        	...
	        	INNER JOIN m_guru c ON a.id_guru = c.id
                -- INNER JOIN tr_ikut_ujian d ON a.id = d.id_tes
	            WHERE jenis_soal='latihan' and (a.nama_ujian LIKE '%" . $search['value'] . "%' OR b.nama LIKE '%" . $search['value'] . "%' OR c.nama LIKE '%" . $search['value'] . "%') " . $wh_1 . ' ORDER BY a.id DESC LIMIT ' . $start . ', ' . $length . '')->result_array();
            // print_r($q_datanya); die();
            $data = array();
            $no   = ($start + 1);
            ...
            foreach ($q_datanya as $d) {
                ...
                $data_ok[4] = $d['jumlah_soal'];
                $data_ok[5] = $d['waktu'] . ' menit';
                $data_ok[6] = '<a href="' . base_url() . 'adm/h_latihan/det/' . $d['id'] . '" class="btn btn-info btn-xs"><i class="glyphicon glyphicon-search" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Lihat Hasil</a>
                         ';
    ...
    {
        $this->cek_aktif();
        cek_hakakses(array('siswa', 'admin'), $this->session->userdata('admin_level'));
    ...
    public function ikuti_latihan()
    {
        ...
        $a['data'] = $this->db->query("SELECT
									...
									INNER JOIN m_guru c ON a.id_guru = c.id
									LEFT JOIN tr_ikut_ujian d ON CONCAT('" . $a['sess_konid'] . "',a.id) = CONCAT(d.id_user,d.id_tes)
                                    WHERE a.jenis_soal='latihan' 
									ORDER BY a.id ASC")->result();
        //echo $this->db->last_query();
        $a['p'] = 'm_list_latihan_siswa';
        $this->load->view('aaa', $a);
    }

    public function ikut_latihan()
    {
        $this->cek_aktif();
        cek_hakakses(array('siswa', 'admin'), $this->session->userdata('admin_level'));
        ...
        if ($uri3 == 'simpan_satu') {
            ...
            $a['du'] = $this->db->query("SELECT a.id, a.tgl_mulai, a.terlambat,
										...
										INNER JOIN m_guru b ON a.id_guru = b.id
										INNER JOIN m_mapel c ON a.id_mapel = c.id
										WHERE a.jenis_soal='latihan' and a.id = '$uri4'")->row_array();
            ...
            if ($cek_sdh_selesai < 1) {
                ...
                if ($cek_sdh_ujian < 1) {
                    $soal_urut_ok = array();
                    $q_soal       = $this->db->query("SELECT id, file, tipe_file, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, '' AS jawaban FROM m_soal WHERE jenis_soal='latihan' and id_mapel = '" . $cek_detil_tes->id_mapel . "' AND id_guru = '" . $cek_detil_tes->id_guru . "' " . $acakan . ' LIMIT ' . $cek_detil_tes->jumlah_soal)->result();
                    $i            = 0;
                    foreach ($q_soal as $s) {
                        $soal_per            = new stdClass();
                        $soal_per->id        = $s->id;
                    ...
                    for ($i = 0; $i < sizeof($urut_soal); ++$i) {
                        $pc_urut_soal   = explode(':', $urut_soal[$i]);
                        $pc_urut_soal1  = empty($pc_urut_soal[1]) ? "''" : "'" . $pc_urut_soal[1] . "'";
                        $ambil_soal     = $this->db->query("SELECT *, $pc_urut_soal1 AS jawaban FROM m_soal WHERE jenis_soal='latihan' and id = '" . $pc_urut_soal[0] . "'")->row();
                        $soal_urut_ok[] = $ambil_soal;
                    }
                ...
                $this->load->view('v_latihan', $a);
            } else {
                redirect('adm/sudah_selesai_latihan/' . $uri4);
            }
        }
    }

    public function sudah_selesai_latihan()
    {
        $this->cek_aktif();
        ...
        if (empty($q_nilai)) {
            redirect('adm/ikut_latihan/_/' . $uri3);
        } else {
            $a['p'] = 'v_selesai_latihan';

            if ($this->config->item('tampil_nilai')) {
                $a['data'] = "<div class='alert alert-danger'>Anda telah selesai mengikuti latihan ini pada : <strong style='font-size: 16px'>" . tjs($q_nilai->tgl_selesai, 'l') . "</strong>, dan mendapatkan nilai : <strong style='font-size: 16px'>" . $q_nilai->nilai . '</strong></div>';
            } else {
                $a['data'] = "<div class='alert alert-danger'>Anda telah selesai mengikuti latihan ini pada : <strong style='font-size: 16px'>" . tjs($q_nilai->tgl_selesai, 'l') . '</strong>. Nilai akan diumumkan oleh Guru yang bersangkutan.</div>';
            }
        }
        $this->load->view('aaa', $a);
    }
    ...
    // fungsi untuk mengaktifkan semua siswa
    public function aktifkan_semua_siswa()
    {
        $siswas = $this->db->select('nim')->select('id')->get('m_siswa')->result();

        // masukkan semua data yang akan diinsert
        $dataInput = [];
        foreach ($siswas as $index => $siswa) {
            $dataInput[$index]['username'] = $siswa->nim;
            $dataInput[$index]['password'] = md5($siswa->nim);
            $dataInput[$index]['level']    = 'siswa';
            $dataInput[$index]['kon_id']   = $siswa->id;
        }

        // $this->Helper->dd($dataInput);
        $this->db->insert_batch('m_admin', $dataInput);

        redirect('adm/m_siswa');
    }

    // fungsi untuk mengaktifkan semua guru
    public function aktifkan_semua_guru()
    {
        $gurus = $this->db->select('nip')->select('id')->get('m_guru')->result();

        // masukkan semua data yang akan diinsert
        $dataInput = [];
        foreach ($gurus as $index => $guru) {
            $dataInput[$index]['username'] = $guru->nip;
            $dataInput[$index]['password'] = md5($guru->nip);
            $dataInput[$index]['level']    = 'guru';
            $dataInput[$index]['kon_id']   = $guru->id;
        }
    ...
    {
        $this->cek_aktif();
        cek_hakakses(['admin', 'guru', 'siswa'], $this->session->userdata('admin_level'));
        ...
        if ('det' == $uri3) {
            $a = $this->db->query("SELECT * FROM m_materi WHERE id = '$uri4' AND id_guru = '$a[sess_konid]' ORDER BY id DESC")->row();
            j($a);
            exit();
        } elseif ('import' == $uri3) {
            $a['p'] = 'f_materi_import';
        } elseif ('hapus_gambar' == $uri3) {
            $nama_gambar = $this->db->query("SELECT file FROM m_soal WHERE id = '" . $uri5 . "'")->row();
            $this->db->query("UPDATE m_soal SET file = '', tipe_file = '' WHERE id = '" . $uri5 . "'");
            @unlink('./upload/gambar_soal/' . $nama_gambar->file);
            redirect('adm/m_soal/pilih_mapel/' . $uri4);
        } elseif ('pilih_mapel' == $uri3) {
            if ('guru' == $a['sess_level']) {
                $a['data'] = $this->db->query("SELECT m_soal.*, m_guru.nama AS nama_guru FROM m_soal INNER JOIN m_guru ON m_soal.id_guru = m_guru.id WHERE m_soal.id_guru = '" . $a['sess_konid'] . "' AND m_soal.id_mapel = '$uri4' ORDER BY id DESC")->result();
            } else {
                $a['data'] = $this->db->query("SELECT m_soal.*, m_guru.nama AS nama_guru FROM m_soal INNER JOIN m_guru ON m_soal.id_guru = m_guru.id WHERE m_soal.id_mapel = '$uri4' ORDER BY id DESC")->result();
            ...
            unset($data_simpan['mode']);
            // print_r($data_simpan); die();
            // $this->db->where("id", $data_simpan['id']);
            $this->db->insert('m_materi', $data_simpan);
            ...
            $data_simpan = $this->input->post();
            $this->db->where('id', $data_simpan['id'])->update('m_materi', $data_simpan);

            echo json_encode(['status' => 'ok']);
            exit;
        } elseif ('edit' == $uri3) {
            // $a['opsij'] = ['' => 'Jawaban', 'A' => 'A', 'B' => 'B', 'C' => 'C', 'D' => 'D', 'E' => 'E'];

            $id_guru = 'guru' == $this->session->userdata('admin_level') ? "WHERE a.id_guru = '" . $a['sess_konid'] . "'" : '';

            $a['p_mapel'] = obj_to_array($this->db->query("SELECT b.id, b.nama FROM tr_guru_mapel a INNER JOIN m_mapel b ON a.id_mapel = b.id $id_guru")->result(), 'id,nama');

            if (0 == $uri4) {
                $a['d'] = ['mode' => 'add', 'id' => '0', 'id_guru' => $id_guru, 'id_mapel' => '', 'bobot' => '1', 'file' => '', 'soal' => '', 'opsi_a' => '#####', 'opsi_b' => '#####', 'opsi_c' => '#####', 'opsi_d' => '#####', 'opsi_e' => '#####', 'jawaban' => '', 'tgl_input' => ''];
            } else {
                $a['d'] = $this->db->query("SELECT m_soal.*, 'edit' AS mode FROM m_soal WHERE id = '$uri4'")->row_array();
            }
            ...
            $this->db->delete('m_materi', ['id' => $this->uri->segment(4)]);

            echo json_encode(['status' => 'ok']);
            exit;
        } elseif ('cetak' == $uri3) {
            $html = "<link href='" . base_url() . "___/css/style_print.css' rel='stylesheet' media='' type='text/css'/> <script defer>print(); close();</script>";

            $materis = $this->db->get("m_materi")->result();

            $html .= '<table>';
            foreach ($materis as $index => $materi) {
                $html .= '<tr>
                            <td width="2%" style="font-weight: bold">' . $no = ++$index . '</td>
                            <td width="2%">' . $this->db->where('id', $materi->id_mapel)->get('m_mapel')->row()->nama . '</td>
                            <td width="2%">' . $this->db->where('id', $materi->id_guru)->get('m_guru')->row()->nama . '</td>
                            <td width="94%">' . $materi->materi . '</td>
                        </tr>';
            }
            $html .= '</table>';
            ...
            $d_total_row = $this->db->query("SELECT id FROM m_materi WHERE materi LIKE '%$search[value]%'")->num_rows();

            $q_datanya = $this->db->query("SELECT materi.id as materi_id, materi.*, guru.id, guru.nama FROM m_materi materi JOIN m_guru guru ON materi.id_guru = guru.id WHERE materi LIKE '%$search[value]%' ORDER BY materi.id DESC LIMIT $start,$length")->result_array();
            $data      = [];
            $no        = ($start + 1);
            ...
            foreach ($q_datanya as $d) {
                ...
                // jika materinya bukan file excel
                // print_r(pathinfo($d['materi'], PATHINFO_EXTENSION)); die();
                if (in_array(pathinfo($d['materi'], PATHINFO_EXTENSION), ['doc', 'docx', 'pdf'])) {
                    $link_download_materi = base_url($d['materi']);
                    $data_ok[2]           = "<a href='{$link_download_materi}' target='_blank' download>DOWNLOAD</a>";
                } else {
                    $materi           = substr($d['materi'], 0, 30);
                    $link_edit_materi = base_url("adm/m_materi_detail/{$d['materi_id']}");
                    $data_ok[2]       = "<a href='{$link_edit_materi}' target='_blank'>{$materi}...</a>";
                }

                if (in_array($this->session->userdata('admin_level'), ['admin', 'guru'])):
                    $data_ok[3] = "<div class='btn-group'>
											                            <a href='#' onclick=\"return m_materi_e('$d[materi_id]');\" class='btn btn-info btn-xs'><i class='glyphicon glyphicon-pencil' style='margin-left: 0px; color: #fff'></i> &nbsp;&nbsp;Edit</a>
											                            <a href='#' onclick=\"return m_materi_h('$d[materi_id]');\" class='btn btn-danger btn-xs'><i class='glyphicon glyphicon-remove' style='margin-left: 0px; color: #fff'></i> &nbsp;&nbsp;Hapus</a>
											                            ";
                endif;
            ...
            $mapels = $this->db->get("m_mapel")->result();
            $gurus  = $this->db->get('m_guru')->result();

            // jika yang login adalah guru
            if (in_array($this->session->userdata('admin_level'), ['guru'])) {
                $mapels = $this->db->query("select * from tr_guru_mapel join m_mapel on tr_guru_mapel.id_mapel = m_mapel.id where id_guru = $a[sess_konid]")->result();
                $gurus  = $this->db->where('id', $a['sess_konid'])->get("m_guru")->result();
            }
            ...
            // jika yang login adalah guru
            $mapels = $this->db->select('id, nama')->get('m_mapel')->result_array();
            $gurus  = $this->db->select('id, nama')->get('m_guru')->result_array();

            if (in_array($this->session->userdata('admin_level'), ['guru'])) {
                $mapels = $this->db->query("select m_mapel.id,m_mapel.nama from tr_guru_mapel join m_mapel on tr_guru_mapel.id_mapel = m_mapel.id where id_guru = $a[sess_konid]")->result_array();
                $gurus  = $this->db->select('id, nama')->where('id', $a['sess_konid'])->get("m_guru")->result_array();
                // print_r($gurus); die();
            }
            ...
            //     $a['mapels'][$key][0] = $mapel['id'];
            //     $a['mapels'][$key][1] = $mapel['nama'];
            // }
            // print_r($a['mapels']);die();
            $a['p'] = 'm_materi';
        }
        $this->load->view('aaa', $a);
    }

    public function m_materi_detail($materi)
    {
        $this->cek_aktif();
        cek_hakakses(['admin', 'guru', 'siswa'], $this->session->userdata('admin_level'));

        $data['data'] = $this->db->where('id', $materi)->get('m_materi')->row();
        $data['p']    = 'm_materi_detail';
        $this->load->view('aaa', $data);
    }
}
