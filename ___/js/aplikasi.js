$(document).ready(function () {
    ...
    // console.log(url2);
    if (url == "m_materi") {
        pagination("datatabel", base_url + "adm/m_materi/data", []);

        if ((url2 == "edit") || (typeof url2 === "undefined")) {
        ...
        pagination("datatabel", base_url + `adm/m_soal/data/${url2}`, []);
    ...
    } else if (url == "m_latihan") {
        pagination("datatabel", base_url + `adm/m_latihan/data`, []);
...
function timer_latihan() {
    ...
    if (statuse == 1) {
        $("#btn_mulai").html('<a href="#" class="btn btn-success btn-lg" id="tbl_mulai" onclick="return konfirmasi_token_latihan(' + id_ujian + ')"><i class="fa fa-check-circle"></i> MULAI</a>');
...
function konfirmasi_token_latihan(id) {
    // alert(url2);
    var token_asli = $("#_token").val();
    var token_input = $("#token").val();
    ...
    if (token_asli != token_input) {
        ...
        alert("Token benar..!");
        window.location.assign(base_url + `adm/ikut_latihan/_/` + id);
    }
}
...
function m_latihan_s() {
    ...
    $.ajax({
        type: "POST",
        url: base_url + "adm/m_latihan/simpan",
        data: JSON.stringify(form),
        dataType: 'json',
        contentType: 'application/json; charset=utf-8'
    }).done(function (response) {
        if (response.status == "ok") {
            window.location.assign(base_url + "adm/m_latihan");
        } else {
            alert(response.caption);
        }
    });
...
function m_latihan_h(id) {
    if (confirm('Anda yakin..?')) {
        $.ajax({
            type: "GET",
            url: base_url + "adm/m_latihan/hapus/" + id,
            success: function (response) {
                if (response.status == "ok") {
                    window.location.assign(base_url + "adm/m_latihan");
                } else {
...
function m_materi_e(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            CKEDITOR.instances['editornya'].setData(data.materi);
        }
    });
    return false;
}
...
function m_materi_s() {
    var f_asal = $("#f_materi");
    var form = getFormData(f_asal);
    form.materi = CKEDITOR.instances['editornya'].getData();

    console.log(form);
    $.ajax({
        type: "POST",
        url: base_url + "adm/m_materi/update",
        data: form,
        success: function (response) {
